...
- TEMPORARY agreement (spec-only; delete once implemented): public API endpoint format will be `POST /api/v1/apps/{appId}/runs` on same-origin (`studio.nexxonn.ai`), with conventions: no trailing slash, JSON, reserve `Authorization` header for API key auth (exact scheme TBD).
...
- Api secret key records live in Nexxonn Storage (one `ApiSecretRecord` per `apiKeyId` at `apiSecretPath(apiKeyId)`; no scans).
...
- Architecture involves `apps` (playground, studio) and `packages` (nexxonn, protocol, etc.).
...
- Implemented `apiPublishing` feature flag:
  - Added `apiPublishingFlag` to `apps/studio.nexxonn.ai/flags.ts`.
  ...
  - Added to data-loader in `apps/studio.nexxonn.ai/app/workspaces/[workspaceId]/data-loader.ts`.
...
- Added Nexxonn Storage path helper for API secrets and started implementing API secret key management in `packages/nexxonn` (token parsing + create/revoke/verify).
- Exposed API secret key management APIs from `@nexxonn-ai/nexxonn` (exports + `nexxonn.createApiSecret` / `nexxonn.revokeApiSecret` methods).
- (Rolled back) Removed Studio API routes for API publishing under `/api/v1/apps/{appId}/...` for now; key generation is handled via the Nexxonn HTTP layer.
...
- Added unit tests for API publishing token parsing, hashing verification, and single-active revoke behavior in `packages/nexxonn`.
- Fixed `pnpm build-sdk` failure: replaced promisified `crypto.scrypt` call with a typed async wrapper for TS DTS generation.
- Added a short comment explaining the `gsk_` token prefix meaning in `packages/nexxonn/src/api-publishing/token.ts`.
- Added `createApiSecret` and `revokeApiSecret` JSON routes to `@nexxonn-ai/http` (`packages/http/src/router.ts`) to match `packages/nexxonn/src/nexxonn.ts`.
- Updated `AGENTS.md` with a rule: when adding a new public API to `packages/nexxonn/src/nexxonn.ts`, also add the corresponding route in `packages/http/src/router.ts`.
- Refactored API publishing UI to use `useNexxon()` client methods instead of manual `fetch()` calls, and added a `getCurrentApiSecretRecordForApp` API/route to support that.
- Fixed build/type errors: removed duplicate export of `getCurrentApiSecretRecordForApp` in `packages/nexxonn/src/api-publishing/api-secrets.ts` and updated studio route to call `nexxonn.getCurrentApiSecretRecordForApp`.
...
- Hardened `lastUsedAt` updates to reduce the chance of overwriting a concurrent revoke.
 - Updated Studio docs to describe `scrypt` env vars for API secret hashing (and removed `NEXXONN_API_SECRET_PEPPER`).
 ...
 - Added `NexxonConfig.apiSecretScrypt` to allow configuring scrypt params + salt size + optional duration logging.
 - Updated `NexxonContext` to carry `apiSecretScrypt` (replacing the removed pepper field).
 - Refactored API publishing secret hashing to be `scrypt`-only, with optional duration logging for observability.
 - Updated `packages/nexxonn` API secret tests to match the `scrypt`-only implementation.
 - Updated Studio Nexxonn initialization to configure `apiSecretScrypt` via env (and removed pepper usage).
 - Updated Playground Nexxonn initialization to configure `apiSecretScrypt` via env (and removed pepper usage).
...
- Updated `@nexxonn-ai/nexxonn` exports to expose API publishing helpers needed by the Studio route.
...
- Current behavior: key records live in Nexxonn Storage (hash-only). `createApiSecret` attempts best-effort single-active by revoking the previous key if present, but verification is record-based and does not enforce “current `app.apiPublishing.apiKeyId` only”.
...
- What are the best `scrypt` parameters for API secret hashing in production (Vercel Functions)? Embed timing instrumentation in the hashing path, observe real numbers in the runtime environment, and decide parameters based on measurements. Ensure parameters are configurable (via `NexxonConfig` / env) so we can adjust them later without a refactor.
...
- `apps/studio.nexxonn.ai/flags.ts`
- `packages/react/src/feature-flags/context.ts`
- `packages/react/src/workspace/provider.tsx`
- `apps/studio.nexxonn.ai/app/workspaces/[workspaceId]/data-loader.ts`
- `AGENTS.md`
- `CONTINUITY.md`
