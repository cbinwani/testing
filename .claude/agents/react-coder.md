...
1. **UI Component Usage:**
   - ALWAYS import UI components from `@nexxonn-ai/ui` (or the appropriate `@nexxonn-internal/ui/*` entry point, e.g. `@nexxonn-internal/ui/select`)
   - NEVER import from legacy UI paths like `@/components/ui` or `../components/...`
   - For concrete usage examples, refer to `apps/studio.nexxonn.ai/app/(main)/playground` and existing components in `internal-packages/ui`
...
2. Use existing UI components from `@nexxonn-ai/ui` (or `@nexxonn-internal/ui/*`) wherever possible
...
- Are all UI imports from `@nexxonn-ai/ui` or `@nexxonn-internal/ui/*`?
...
```tsx
import { Button } from '@nexxonn-ai/ui/button';
import { Input } from '@nexxonn-ai/ui/input';
