# AGENTS.md - Nexxonn Development Guide
...
Nexxonn is built to design and run AI workflows beyond prompt chains. Not a chat. Not a chain. A system you can run.
...
Nexxonn uses a **Turborepo monorepo** with pnpm workspaces, organized into four main directories:

```
/workspace
├── apps/                    # Deployable applications
│   ├── playground/          # Self-hosted version (experimental)
│   ├── studio.nexxonn.ai/  # Nexxonn Cloud (production)
│   └── ui.nexxonn.ai/      # UI component showcase
├── packages/                # Published SDK packages (@nexxonn-ai/*)
├── internal-packages/       # Internal shared packages (@nexxonn-internal/*)
└── tools/                   # Development utilities
```

### Package Layers

**SDK Packages (`packages/@nexxonn-ai/*`):**
- `protocol` — Core domain types and schemas (Workspace, Node, Task, Generation)
- `nexxonn` — Engine implementation (tasks, generations, triggers, integrations)
...
**Internal Packages (`internal-packages/@nexxonn-internal/*`):**
...
```
Workspace (JSON) → Protocol Types → Nexxonn Engine → Task Execution → Generation Output
                                          ↓
...
pnpm dev:studio.nexxonn.ai  # Start Nexxonn Cloud

# Build
pnpm build-sdk               # Build SDK packages
pnpm -F playground build     # Build playground app
pnpm -F studio.nexxonn.ai build  # Build Nexxonn Cloud
...
### API addition rule (Nexxonn ↔ HTTP)

When adding a new **public API** to `packages/nexxonn/src/nexxonn.ts`, also add the corresponding routing entry to `packages/http/src/router.ts` (typically `jsonRoutes.<name>` using `nexxonn.<name>.inputSchema`) so the API is reachable through the HTTP layer (e.g., via `NextNexxon`).

### Testing

```sh
pnpm test                           # Run all tests
pnpm -F @nexxonn-ai/nexxonn test   # Run tests for a specific package
cd packages/nexxonn && vitest       # Run tests in watch mode
...
```typescript
const marker = "nexxonn.react.error.APICallError";
...
```typescript
try {
  return await jsonRoutes[routerPath](nexxonn)({ input });
} catch (e) {
...
**Step 1: Define the flag in `apps/studio.nexxonn.ai/flags.ts`**
