import type { AiGatewayHeaders, QueryContext } from "@nexxonn-ai/nexxonn";
import { traceEmbedding } from "@nexxonn-ai/langfuse";
import { getRequestId, NextNexxonn } from "@nexxonn-ai/nextjs/internal";
import { type RunningGeneration, WorkspaceId } from "@nexxonn-ai/protocol";
import type { EmbeddingMetrics } from "@nexxonn-ai/rag";
import {
	supabaseStorageDriver as experimental_supabaseStorageDriver,
	supabaseVaultDriver,
} from "@nexxonn-ai/supabase-driver";
	...
	callbacks: {
		...
		buildAiGatewayHeaders: ({ metadata, generation }) => {
			...
			const aiGatewayHeaders: AiGatewayHeaders = {
				"http-referer":
					process.env.AI_GATEWAY_HTTP_REFERER ?? "https://nexxonn.ai",
				"x-title": process.env.AI_GATEWAY_X_TITLE ?? "Nexxonn",
			};
...
if (generateContentProcessor === "trigger.dev") {
	nexxonn.setGenerateContentProcess(async ({ generation, metadata }) => {
		const requestId = getRequestId();
	...
	nexxonn.setRunTaskProcess(async ({ task, generationOriginType }) => {
		const requestId = getRequestId();
