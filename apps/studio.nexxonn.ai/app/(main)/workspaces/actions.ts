...
import { AppId, isAppEntryNode, isTriggerNode } from "@nexxonn-ai/protocol";
import type { AgentId } from "@nexxonn-ai/types";
...
export async function copyAgent(
	...
	try {
		...
		let workspace = await nexxonn.copyWorkspace(agent.workspaceId, newName);
		...
		const nodesWithDuplicatedApps = await Promise.all(
			workspace.nodes.map(async (node) => {
				...
				const sourceApp = await nexxonn.getApp({
					appId: node.content.appId,
				});

				const newAppId = AppId.generate();
				await nexxonn.saveApp({
					app: {
						...sourceApp,
						id: newAppId,
						entryNodeId: node.id,
		...
		if (hasAppDuplications) {
			...
			workspace = await nexxonn.updateWorkspace(updatedWorkspace);
		}
		...
		for (const node of workspace.nodes) {
			...
			const trigger = await nexxonn.getTrigger({
