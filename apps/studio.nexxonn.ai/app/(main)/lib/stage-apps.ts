import { isTextGenerationNode, isVectorStoreNode } from "@nexxonn-ai/protocol";
...
async function getAppsBySampleFlag(
	...
	const result = await Promise.all(
		dbWorkspaces.map(async (dbWorkspace) => {
			const nexxonnWorkspace = await nexxonn.getWorkspace(dbWorkspace.id);
			const appEntryNode = nexxonnWorkspace.nodes.find(
			...
			const nexxonnApp = await nexxonn.getApp({
			...
			for (const connection of nexxonnWorkspace.connections ?? []) {
			...
			for (const node of nexxonnWorkspace.nodes) {
	...
	for (const data of result) {
		...
		apps.push({
			id: data.nexxonnApp.id,
			name: data.workspace.name ?? "Untitled",
			description: data.nexxonnApp.description,
			entryNodeId: data.nexxonnApp.entryNodeId,
			parameters: data.nexxonnApp.parameters,
