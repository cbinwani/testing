import { defaultName } from "@nexxonn-ai/node-registry";
import {
	isTriggerNode,
	type Workspace,
	type WorkspaceId,
...
async function enrichActWithNavigationData(
	...
	try {
		const tmpAct = await nexxonn.getTask({ taskId: act.sdkActId });
		const team = teams.find((t) => t.dbId === act.teamDbId);
		if (team === undefined) {
			throw new Error("Team not found");
		}
		const tmpWorkspace = await nexxonn.getWorkspace(act.sdkWorkspaceId);
	...
	for (const team of teams) {
		...
		for (const tmpFlowTrigger of tmpFlowTriggers) {
			if (!workspaceMap.has(tmpFlowTrigger.sdkWorkspaceId)) {
				const tmpWorkspace = await nexxonn.getWorkspace(
			...
			const flowTrigger = await nexxonn.getTrigger({
