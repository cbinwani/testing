...
import { getNexxonnSession, updateNexxonnSession } from "@/lib/nexxonn-session";
import { stripe } from "@/services/external/stripe";
import { getLatestSubscriptionV2 } from "@/services/subscriptions/get-latest-subscription-v2";

export async function GET(_request: Request) {
	const session = await getNexxonnSession();
	...
	try {
		...
		await updateNexxonnSession({ teamId, checkoutSessionId: undefined });
	} catch (error) {
		// fallback
