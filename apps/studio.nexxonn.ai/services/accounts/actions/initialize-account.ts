...
import { App, AppId, NodeId } from "@nexxonn-ai/protocol";
...
export const initializeAccount = async (
	...
	const result = await db.transaction(async (tx) => {
		...
		// create sample apps
		const sampleWorkspaceResults = await nexxonn.createSampleWorkspaces();
		...
		for (const result of sampleWorkspaceResults) {
			...
			if (templateAppInfo) {
				...
				if (newEntryNodeId) {
					...
					const templateAppJson = await nexxonn.getApp({
						appId: templateAppInfo.id,
					});

					// Create new app JSON with mapped node IDs and save directly to storage
					// (not using nexxonn.saveApp() because it triggers appCreate callback which inserts DB record)
					const storage = nexxonn.getContext().storage;
