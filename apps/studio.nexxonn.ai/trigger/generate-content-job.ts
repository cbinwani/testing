import { GenerationId, isRunningGeneration } from "@nexxonn-ai/protocol";
...
export const generateContentJob = schemaJob({
	...
	run: async (payload) => {
		const generation = await nexxonn.getGeneration(payload.generationId);
		if (!isRunningGeneration(generation)) {
			return {
				message: `Generation ${payload.generationId} is not running.`,
			};
		}
		await nexxonn.generateContent({
			generation,
			logger,
			metadata: {
