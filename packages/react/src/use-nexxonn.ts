...
export type NexxonnRequestOptions = {
	signal?: AbortSignal;
	keepalive?: boolean;
};
...
/**
 * Custom hook that provides a type-safe client for the Nexxonn API
 *
 * @param options Configuration options for the client
 * @returns A client object with methods for each Nexxonn operation
 */
export function useNexxonn(options?: FetchOptions): NexxonnClient {
	const basePath = options?.basePath ?? "/api/nexxonn";
	...
	const client = useMemo(() => {
			...
			},
			{
				get: (target, prop) => {
					...
					if (typeof prop === "string") {
						...
						return (input?: unknown, options?: NexxonnRequestOptions) =>
		...
		return proxyClient as NexxonnClient;
