...
} from "@nexxonn-ai/github-tool";
import {
	formDataRoutes,
	isFormDataRoutePath,
	isJsonRoutePath,
	jsonRoutes,
} from "@nexxonn-ai/http";
import { after } from "next/server";
import { ZodError } from "zod";
import { type RequestContext, requestContextStore } from "./context";

interface NextNexxonnConfig extends Omit<NexxonnConfig, "waitUntil"> {
	basePath: string;
	...
	return async function httpHandler(request: Request) {
		...
		if (config.onRequest) {
			await config.onRequest({
				...
				updateContext: (updates) => nexxonn.updateContext(updates),
			});
		}
		...
		return await requestContextStore.run(ctx, async () => {
			...
			if (generatedImageMatch) {
				...
				const file = await nexxonn.getGeneratedImage(
			...
			if (config.useAfterFunction) {
				...
				after(async () => {
					await nexxonn.flushGenerationIndexQueue();
				});
			}

			if (isJsonRoutePath(routerPath)) {
				try {
					return await jsonRoutes[routerPath](nexxonn)({
						// @ts-expect-error
			...
			if (isFormDataRoutePath(routerPath)) {
				return await formDataRoutes[routerPath](nexxonn)({
					// @ts-expect-error
					input: await getBody(request),
				});
			}
			/** Handle GitHub webhooks with Nexxonn */
...
export function NextNexxonn(config: NextNexxonnConfig) {
	const nexxonn = Nexxonn({ ...config, waitUntil: after });
