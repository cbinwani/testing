import type { Nexxonn } from "@nexxonn-ai/nexxonn";
import { memoryStorageDriver } from "@nexxonn-ai/storage";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { createHttpHandler } from "./next-nexxonn-engine";

// Mock the module dependencies
vi.mock("@nexxonn-ai/http", () => {
...
describe("createHttpHandler", () => {
	...
	} as unknown as Nexxonn;
	const basePath = "/api/nexxonn";
	...
	it("should handle generated image multi-segment path", async () => {
		...
		expect(mockNexxonn.getGeneratedImage).toHaveBeenCalledWith(
			generationId,
			filename,
		);
