...
function getApiSecretScryptSaltBytes(context: NexxonnContext): number {
...
async function getAppOrThrow(context: NexxonnContext, appId: AppId) {
...
export const createApiSecret = createNexxonnFunction({
	input: z.object({
		appId: AppId.schema,
	}),
...
export const revokeApiSecret = createNexxonnFunction({
	input: z.object({
		appId: AppId.schema,
	}),
	handler: async ({ context, input }) => {
...
export const getCurrentApiSecretRecordForApp = createNexxonnFunction({
	input: z.object({
		appId: AppId.schema,
	}),
	handler: async ({ context, input }) => {
