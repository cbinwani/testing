import { type NexxonnLogger, noopLogger } from "@nexxonn-ai/logger";
import {
...
import { bindNexxonnFunction } from "./utils/create-nexxonn-function";
import {
	copyWorkspace,
...
export function Nexxonn(config: NexxonnConfig) {
	const context: NexxonnContext = {
		...config,
	...
	return {
		updateContext: (updates: Partial<NexxonnContext>): void => {
			Object.assign(context, updates);
		},
		getContext: (): NexxonnContext => context,
		...
		createApiSecret: bindNexxonnFunction(createApiSecret, context),
		getCurrentApiSecretRecordForApp: bindNexxonnFunction(
			getCurrentApiSecretRecordForApp,
			context,
		),
		revokeApiSecret: bindNexxonnFunction(revokeApiSecret, context),
		saveApp: bindNexxonnFunction(saveApp, context),
		deleteApp: bindNexxonnFunction(deleteApp, context),
		getApp: bindNexxonnFunction(getApp, context),
	};
}

export type Nexxonn = ReturnType<typeof Nexxonn>;
