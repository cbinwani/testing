# @nexxonn-ai/document-preprocessor
...
This package is published inside the monorepo under the `@nexxonn-ai/*` namespace. It relies on optional native bindings shipped with PDFium, so make sure your runtime matches the supported Node.js versions (>=18).
...
import { extractText } from "@nexxonn-ai/document-preprocessor";
...
import { extractPdfText } from "@nexxonn-ai/document-preprocessor";
...
import { readFile } from "node:fs/promises";
import { createRequire } from "node:module";
import { renderPdfPageImages } from "@nexxonn-ai/document-preprocessor";
...
```bash
pnpm -F @nexxonn-ai/document-preprocessor test
pnpm -F @nexxonn-ai/document-preprocessor check-types
pnpm -F @nexxonn-ai/document-preprocessor build
```
