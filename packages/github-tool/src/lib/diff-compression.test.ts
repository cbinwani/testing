...
describe("diff compression", () => {
	...
	it("should preserve file headers in compressed output", () => {
		...
		expect(compressed).toContain(
			"diff --git a/apps/studio.nexxonn.ai/app/api/vector-stores/github/ingest/ingest-github-repository.ts",
		);
		...
		expect(compressed).toContain(
			"+++ b/apps/studio.nexxonn.ai/app/api/vector-stores/github/ingest/ingest-github-repository.ts",
		);
	});
